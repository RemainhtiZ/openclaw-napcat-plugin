...
{
  "enableGroupMessages": true,
  "groupKeywords": ["小余", "帮忙", "问一下"]
}
```

适合：不想被 @，但希望群友用特定关键词触发机器人。

配置后，群里只要消息中包含任意一个关键词（不区分大小写），机器人就会处理这条消息。
也可以和 @机器人 一起使用：只要满足"被 @"**或**"包含关键词"任一条件就会回复。
...
| `groupKeywords` | string[] | 群消息关键词触发模式：消息包含任意关键词时也会处理 | `[]` |
